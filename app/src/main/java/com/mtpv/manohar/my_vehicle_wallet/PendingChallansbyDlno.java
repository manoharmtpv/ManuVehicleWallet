...
public class PendingChallansbyDlno extends Fragment {
...
//		title1 = (TextView)findViewById(R.id.title2);
        ...
        vehicle_no.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS);
        vehicle_no.setFilters(new InputFilter[] { filter, new InputFilter.LengthFilter(20) });

        vehicle_heading.setText("Driving License Details");
        heading.setText("Enter Driving Licence Number");
        vehicle_no.setHint("Enter Driving Licence Number");
        vehicleNo_text.setText("Driving Licence No");
        vehicle_no.setText("");
        ...
        resend_captcha.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Async_task_Licence async_task_licence=new Async_task_Licence();
                async_task_licence.execute();
    ...
    public class Async_task_Licence extends AsyncTask<Void, Void, String> {
        String getRegNo = vehicle_no.getText().toString().trim();
        ...
        protected String doInBackground(Void... params) {
            ...
            ServiceHelper.GetViolationsbyDL(getRegNo,Vehicle_Wallet_Login.usertype,captcha_received,Vehicle_Wallet_Login.Mobilenum);
            return null;
        }
        ...
        protected void onPostExecute(String result) {
            ...
            if (ServiceHelper.dl_Response.equals("1")){
                showtoast("Validation","Invalid Captcha");
                captcha_text.setText("");
                table_layout.removeAllViews();
                scroll.setVisibility(View.GONE);
            }else if (ServiceHelper.dl_Response.equals("2")) {
                showtoast("Validation","Invalid Vehicle Details");
                vehicle_no.setText("");
                table_layout.removeAllViews();
                scroll.setVisibility(View.GONE);
            }else if (ServiceHelper.dl_Response.equals("0")) {
                showtoast("Validation","No Details Found");
                vehicle_no.setText("");
                table_layout.removeAllViews();
                scroll.setVisibility(View.GONE);
            }else if (ServiceHelper.Opdata_Chalana.toString().trim().equals("anyType{}")) {
                showtoast("Validation","Please Try After SomeTime");
                vehicle_no.setText("");
                table_layout.removeAllViews();
                scroll.setVisibility(View.GONE);
            }else if (ServiceHelper.Opdata_Chalana.equals("3")) {
                ...
                String strJson = ServiceHelper.dl_Response;
                try {
                            ...
                            if (j == 1) {
                                ...
                                tv.setOnClickListener(new View.OnClickListener() {
                                    ...
                                    public void onClick(View arg0) {
                                        ...
                                        OWNER_NAME = tv_name.getText().toString().trim();
                                        ETICKET_NO = tv_ticket_no.getText().toString().trim();
                                        OFFENCE_DATE = tv_offe_date.getText().toString().trim();
                                        OFFENCE_TIME = tv_offec_time.getText().toString().trim();
                                        PS_NAME = tv_ps_nam.getText().toString().trim();
                                        POINT_NAME = tv_poi_name.getText().toString().trim();
                                        VIOLATION = tv.getText().toString().trim();
                                        VIOLATION_AMT = tv_violat_amt.getText().toString().trim();
                                        ACTUAL_FINE_AMT = tv_actual_fine.getText().toString().trim();
                                        USER_CHARGES = tv_user_charges_dat.getText().toString().trim();
                                        CMD_FINE_AMT = tv_cmd_fin_amt.getText().toString().trim();
                                        LATTITUDE = tv_latt_data.getText().toString().trim();
                                        LONGITUDE = tv_longit_data.getText().toString().trim();
                                        IMAGE_URL = tv_img_url.getText().toString().trim();
                                        VEHICLE_No = vehicle_no.getText().toString().trim();
